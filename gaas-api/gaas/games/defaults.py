class GameSpec(object):
    ...
    def validate_params(self, params, game):
        constrainsts = self.get_param_constraints()
        expected={p.id: constrainsts[p.id] for p in self.params}
        for arg in constrainsts:
            validated = 0
            for parm in params.items():
                print(arg, parm)
                if arg in parm:
                    validated = 1
                    continue
                else:
                    for param in game.params:
                        if arg == param.id and param.optional == True:
                            validated = 1
            if not validated:
...
class Param(object):
    ...
    def __init__(self, id, type, name, description, elements=None, optional=False, default=None):
